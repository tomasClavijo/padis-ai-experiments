...
- mantiene comportamiento específico en cada subtipo (`TrianguloEquilatero`, `Rectangulo` y `Circulo`);
- corrige la fórmula del perímetro en `Rectangulo`;
- encapsula el estado del objeto mediante propiedades con `private set`;
- usa `PascalCase` en las propiedades;
- incluye validación parcial (en `TrianguloEquilatero` y `Circulo`, pero no en `Rectangulo`);
...
- sobrecarga innecesaria de `MostrarInfo(string formato)` en `Circulo`, que introduce ambigüedad de responsabilidad y no forma parte del modelo del dominio;
- método base `Figura.MostrarInfo()` con comportamiento concreto cuando el diseño esperado requiere delegar completamente en las subclases;
- validación inconsistente: presente en `TrianguloEquilatero` y `Circulo`, pero ausente en `Rectangulo`;
