**Guía de Evaluación – Programación Orientada a Objetos**

Actuá como DOCENTE/TUTOR de Programación Orientada a Objetos.

Vas a evaluar la solución del estudiante utilizando el CÓDIGO ACTUAL del proyecto (archivos abiertos en el repositorio).  
No ejecutes el programa: basate únicamente en la lectura del código.
...
**1)**  
Crear un proyecto de librería de clases de nombre Dominio y agregar una clase `Vehiculo`.
...
**2)**  
Dentro del mismo proyecto agregar dos clases, `Auto` y `Camioneta` (que heredan de `Vehiculo`), y definir el método `Encender()` para que imprima:
...
**3)**  
En el proyecto de consola:

- Crear una lista de `Vehiculo`
- Inicializarlos
- Encenderlos
